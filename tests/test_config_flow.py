...
async def test_user_step_single_account(hass: HomeAssistant) -> None:
    ...
    with patch(
        "custom_components.nationalgrid.config_flow.NationalGridApiClient",
    ) as mock_cls:
...
async def test_user_step_communication_error(hass: HomeAssistant) -> None:
    ...
    with patch(
        ...
        client = mock_cls.return_value
        client.async_get_linked_accounts = AsyncMock(
            side_effect=NationalGridApiClientCommunicationError("Timeout"),
        )
        client.close = AsyncMock()
...
async def test_user_step_unknown_error(hass: HomeAssistant) -> None:
    ...
    with patch(
        ...
        client = mock_cls.return_value
        client.async_get_linked_accounts = AsyncMock(
            side_effect=NationalGridApiClientError("Something broke"),
        )
        client.close = AsyncMock()
